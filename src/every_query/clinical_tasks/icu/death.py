...
These carry no ``TIMELINE//END`` censoring guard.  Death terminates the record, so
``TIMELINE//END`` is stamped at the moment of death and such a guard would exclude
exactly the patients who died, which is the positive class.  Censoring is left to the
labeler's own rule, under which a subject dead by the window end is never censored
(#278).
"""
