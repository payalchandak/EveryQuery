...
def create_eq_task_df(eq_shard_fp:str, aces_shard_fp:str, codes:list[str], output_root:str, target_rows:int) -> int:
    # boolean value in aces_shard_df is the real label for the task that will be used for eval
    # Note: every query expected this to eventually be called the "occurs" column as boolean_value is for censoring
    ...
    eq_shard_all_tasks_df = pl.read_parquet(eq_shard_fp,columns=["subject_id","prediction_time","censored",*codes])
...
def main(cfg: DictConfig) -> None:
    ...
    eq_ids = [int(p.stem) for p in all_eq_shards]
    aces_ids = [int(p.stem) for p in all_aces_shards]

    assert eq_ids == aces_ids, f"Shard mismatch: EQ={eq_ids[:5]} ACES={aces_ids[:5]}"
